  ...
  try {
    ...
    if (result instanceof Database) {
      throw new Error.ReqlRuntimeError(
        'Query result must be of type DATUM, GROUPED_DATA, or STREAM (got DATABASE)',
      );
    }
  ...
  switch (termType) {
    ...
    case termTypes.VAR: // 10
      ...
      if (this.context[varId] === undefined) {
        throw new Error.ReqlRuntimeError(
          'The server is buggy, context not found',
        );
      }
    ...
    case termTypes.LE:
      ...
      for (var i = 1; i < term[1].length; i++) {
        right = this.evaluate(term[1][i]);
        if (
          helper.lt(left, right) === false &&
          helper.eq(left, right) === false
        ) {
          return false;
        }
      }
        ...
        if (doc[field] === undefined) {
          throw new Error.ReqlRuntimeError(
            'No attribute `' +
              field +
              '` in object:\n' +
              JSON.stringify(helper.toDatum(doc), null, 2),
            this.frames,
          );
        }
        return doc[field];
    ...
    case termTypes.OBJECT:
      if (term[1].length % 2 === 1) {
        throw new Error.ReqlRuntimeError(
          'OBJECT expects an even number of arguments (but found ' +
            term[1].length +
            ')',
        );
      }
    ...
    case termTypes.WITHOUT:
      ...
      if (
        sequenceOrObject instanceof Sequence ||
    ...
    case termTypes.COERCE_TO:
      ...
      if (newType === 'NUMBER') {
        ...
        if (typeof value.toSequence === 'function') {
          ...
          throw new Error.ReqlRuntimeError(
            'Cannot coerce ' + currentType + ' to ARRAy',
            this.frames,
          );
        }
      } else {
    ...
    case termTypes.ASC:
      ...
      throw new Error.ReqlRuntimeError(
        'Local server is buggy - not reachable code',
      );
    case termTypes.DESC:
      // This should not be evaluated, orderBy will handle it
      throw new Error.ReqlRuntimeError(
    ...
    case termTypes.SECONDS:
      ...
      var dateStr = helper.dateToString(date);
      return parseFloat(
        dateStr.match(/.* .* .* \d{2}:\d{2}:(\d{2}\.{0,1}\d*)/)[1],
        10,
      );
