...
function Table(name, db, options) {
  ...
  this.indexes[this.options.primaryKey] = {
    ...
    fn: [
      ...
      [
        [2, [1]],
        [31, [[10, [1]], this.options.primaryKey]],
      ],
    ],
    options: {},
  };
...
Table.prototype.getAll = (args, options, query) => {
  ...
  if (
    helper.isPlainObject(options) === false ||
  ...
  for (var i = 0; i < keys.length; i++) {
    // Iterate on all the documents of the tablepay attention to what browserify does here
  ...
  selection.selection.sort((left, right) => {
    ...
    if (Array.isArray(options.index) || typeof options.index === 'string') {
      if (Array.isArray(options.index)) {
        if (options.index[0] === 73) {
          // ASC
          index = options.index[1][0];
          order = 'ASC';
        } else if (options.index[0] === 74) {
          // DESC
          index = options.index[1][0];
          order = 'DESC';
        } else {
    ...
    while (index <= fields.length) {
      ...
      if (Array.isArray(field) && field[0] === 73) {
        // ASC
        field = field[1][0];
        order = 'ASC';
      } else if (Array.isArray(field) && field[0] === 74) {
        // DESC
        field = field[1][0];
        order = 'DESC';
      } else {
