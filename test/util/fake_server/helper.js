...
helper.assertType = function assertType(value, type, query) {
  ...
  if (type === 'STRING') {
    if (typeof value !== 'string') {
      throw new Error.ReqlRuntimeError(
        'Expected type STRING but found ' + typeValue,
        query.frames,
      );
    }
  } else if (type === 'NUMBER') {
    if (typeof value !== 'number') {
      throw new Error.ReqlRuntimeError(
        'Expected type NUMBER but found ' + typeValue,
        query.frames,
      );
    }
  } else if (type === 'TIME') {
    ...
    if (!(value instanceof Sequence)) {
      throw new Error.ReqlRuntimeError(
        'Expected type ARRAY but found ' + typeValue,
        query.frames,
      );
    }
  } else if (type === 'INT') {
...
helper.assertNoSpecialChar = function assertNoSpecialChar(value, type, query) {
  if (!value.match(/^[0-9a-zA-Z]+$/)) {
    throw new Error.ReqlRuntimeError(
      type + ' name `' + value + '` invalid (Use A-Za-z0-9_ only) ',
      query.frames,
    );
  }
};
...
helper.mergeWriteResult = (left, right) => {
  ...
  if (left.generated_keys !== undefined || right.generated_keys !== undefined) {
    if (
      Array.isArray(left.generated_keys) &&
      Array.isArray(right.generated_keys)
    ) {
        ...
        if (Array.isArray(obj[key])) {
          if (self[key].length !== obj[key].length) {
            ...
            for (var i = 0; i < self[key].length; i++) {
              if (
                helper.isPlainObject(self[key][i]) &&
                helper.isPlainObject(obj[key][i])
              ) {
...
helper.makeInternalPk = (value) => {
  ...
  if (typeof value === 'string') {
    ...
    throw new Error.ReqlRuntimeError(
      'Primary keys must be either a number, bool, pseudotype or string (got type ' +
        type +
        '):\n' +
...
helper.validDate = (date) => {
  ...
  if (typeof date.epoch_time !== 'number') {
    throw new Error.ReqlRuntimeError(
      'RqlRuntimeError: Invalid time object constructed (no field `epoch_time`):\n' +
        JSON.stringify(date, null, 2),
    );
  } else if (
    date.timezone.match(/^(?:Z|[+-](?:2[0-3]|[01][0-9]):[0-5][0-9])$/) == null
  ) {
    throw new Error.ReqlRuntimeError(
      'RqlRuntimeError: Invalid time object constructed (no field `timezone`):\n' +
        JSON.stringify(date, null, 2),
    );
  }
};
...
helper.dateToString = (date) => {
  ...
  var timezone_int =
    (parseInt(timezone_array[0], 10) * 60 + parseInt(timezone_array[1], 10)) *
    60;
  if (sign === '-') {
  ...
  if (left instanceof Sequence) {
    if (right instanceof Sequence) {
      ...
      for (var i = 0; i < left.length; i++) {
        ...
        if (
          helper.eq(left[i], right[i]) === false &&
      ...
      if (helper.isPlainObject(right) && right.$reql_type$ === 'TIME') {
        ...
        for (var i = 0; i < leftKeys.length; i++) {
          if (leftKeys[i] !== rightKeys[i]) {
            ...
            helper.lt(left[leftKeys[i]], right[leftKeys[i]]) === false
          ) {
            return false;
          }
        }
        ...
        if (doc[key] === undefined) {
          throw new Error.ReqlRuntimeError(
            'No attribute `' +
              field +
              '` in object:\n' +
              JSON.stringify(doc.toDatum(), null, 2),
            this.frames,
          );
        }
